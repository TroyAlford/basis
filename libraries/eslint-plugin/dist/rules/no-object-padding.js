    ...
    create(context) {
        return {
            ObjectExpression(node) {
                ...
                if (hasLeading) {
                    context.report({
                        fix: fixer => fixer.replaceTextRange([open.range[0] - 1, first.range[0]], opening.replace(WHITESPACE_LINE, '')),
                ...
                if (hasTailing) {
                    context.report({
                        fix: fixer => fixer.replaceTextRange([last.range[1] - 1, close.range[0]], closing.replace(WHITESPACE_LINE, '')),
