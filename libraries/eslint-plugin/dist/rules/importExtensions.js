...
const stringifyPath = ({ hash, pathname, query }) => `${pathname}${query}${hash}`;
const isScoped = (name) => /^@[^/]+\/?[^/]+/.test(name);
const isExternalRootModule = (name) => {
    if (name === '.' || name === '..')
        return false;
    const slashCount = name.split('/').length - 1;
    return slashCount === 0 || (isScoped(name) && slashCount <= 1);
};
const replaceImportPath = (sourceText, importPath) => sourceText.replace(/^(['"])(.+)\1$/, (_match, quote) => `${quote}${importPath}${quote}`);
const getModifier = (extension) => POLICY[extension] ?? 'never';
